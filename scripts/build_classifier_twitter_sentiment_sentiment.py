...
Uses as dataset our collected Twitter data and exports extracted features to
arff format as Weka input. Sentiment analysis task is comprised of 2 parts:
1. predict if tweet is relevant/irrelevant
2. if relevant, predict sentiment
This script addresses part 2 of the sentiment prediction task, i.e. it only
predicts sentiment for relevant tweets.
Weka prediction results are stored under
weka_predictions/sentiment_sentiment_predictions_twitter
...
from weka_functions import get_top_k_weka_predictions
...
def add_features(df, is_training_set=True, glove_src="", use_tfidf=False):
    ...
    if is_training_set:
        ...
        if use_tfidf:
            ...
            # import json
            #
            # with codecs.open("tfidf_weights_twitter.json", "w") as fp:
...
def read_as_df_gizem(src, src_pred, binary=False):
    """
    ...
    ----------
    src: str - path to dataset.
    src_pred: str - path to Weka's predictions.
    binary: bool - True if <src_pred> contains predictions for a binary
    classification task. Otherwise it's assumed that it involves 3 classes.
    ...
    """
    # Relevance classification is a binary task
    preds = get_top_k_weka_predictions(src_pred, 2, binary=binary)
    tweets = read_json(src)
    tids = []
    texts = []
    sentiments = []
    ...
    cnt = 0
    for tid in tweets:
        # Keep only tweets that are considered "Relevant"
        if preds[tid][PREDICTED_LABEL] == "Relevant":
            tids.append(tid)
            t = tweets[tid]
            # Get certainty of highest class
            most_certain = preds[tid][TOP_PROBAS][0]
            certains.append(most_certain)
            margins.append(preds[tid][MARG])
            ...
            cnt += 1
    print "tweets predicted as irrelevant", cnt
    data = {
        TEXT: texts,
        LABEL: labels,
        SENTIMENT: sentiments,
...
def export_to_weka(src, glove_src, use_tfidf, dst, features, label):
    ...
    df = add_features(df, is_training_set=True, glove_src=glove_src,
                      use_tfidf=use_tfidf)
    to_arff(df, features, label, dst)


def export_to_weka_gizem(src, src_pred, glove_src, use_tfidf, dst, features,
                         label,
                   training=True):
    """
    ...
    src_pred: str - path to Weka's predictions to figure out which tweets were
    ...
    """
    # Read dataset
    df = read_as_df_gizem(src, src_pred, binary=True)
    # Preprocess tweets
    df = clean_tweets(df)
    # Transform data into matrix representation adding all features
...
if __name__ == "__main__":
    ...
    # Directory in which the glove vectors existing in the twitter dataset will
    # be stored
    ...
    GLOVE_W2V_VEC_WIKI = os.path.join(TWITTER_DIR, "glove.42B.300d_glove_for_twitter_watson_rosette_min_annos_3_cleaned.json")
    # Trained on Twitter
    ...
    # Directory in which Weka predictions for relevance of the tweets w.r.t.
    # sentiment analysis task are stored
    PRED_DIR = os.path.join(base_dir, "results", "weka_predictions",
    ...
    if not os.path.isfile(src):
        raise IOError("Dataset doesn't exist - run 'create_dataset.py' first!")
    ...
    # vecs = read_glove_vectors(GLOVE_W2V_VEC2_TWITTER)
    #
    ...
    #################################################
    # Now perform Weka experiments with the dataset #
    #################################################
    # a) Formulate problem as a regression task
    # Create dataset once using Wikipedia glove vectors
    # fname = "dataset_twitter_wikipedia_regression.arff"
    ...
    # fname = "dataset_twitter_twitter_regression.arff"
    # dst = os.path.join(ARFF_DIR, fname)
    ...
    # Create dataset once using Wikipedia glove vectors
    fname = "sentiment_relevance_twitter_training.csv"
    rel_src = os.path.join(PRED_DIR, fname)
    fname = "dataset_twitter_sentiment_sentiment_twitter_classification.arff"
    dst = os.path.join(ARFF_DIR, fname)
    export_to_weka_gizem(src, rel_src, GLOVE_W2V_VEC_TWITTER, use_tfidf, dst,
                         FEATURES, LABEL, training=True)
    ...
    #
    # # Trained on Wikipedia
    # GLOVE_W2V_VEC_WIKI = os.path.join(TWITTER_DIR,
    #                                   "glove.42B.300d_glove_for_twitter_full_watson_rosette_min_annos_3_cleaned.json")
    # # Trained on Twitter
    ...
    # # TODO: use .json file here for fname that was created in create_twitter_dataset.py
    # fname = "tweets.csv"
    # src = os.path.join(FULL_DIR, fname)
    #
    # # And create dataset once using Twitter glove vectors
    # fname = "dataset_twitter_twitter_classification.arff"
    # dst = os.path.join(ARFF_DIR, fname)
    # export_to_weka(src, GLOVE_W2V_VEC_TWITTER, use_tfidf, dst, FEATURES,
    #                LABEL)
    #
    # # And create dataset once using Twitter glove vectors
    # fname = "dataset_twitter_full_sentiment_sentiment_twitter_classification" \
    #         ".arff"
    # dst = os.path.join(ARFF_DIR, fname)
    # export_to_weka(src, GLOVE_W2V_VEC_TWITTER, use_tfidf, dst, FEATURES,
    #                LABEL)
