"""
Computes how many tweets would be available when extending experiment 3
(which analyzes how the percentage of tweets with low agreement affects
classifier performance). Then the arff files for this experiment are
created.
The way to perform this experiment would be as follows:
1. identify the set of tweets from LOW that never (not when using 4, 5, 6, 7,
and 8 labels) exhibits high-disagreement. Let's call it LD. This determines
the overall size of our dataset (in experiment 3 we used 2.2k tweets, but here
the number will be much lower, perhaps 200 tweets).
2. Identify the set of tweets from LOW that always (using 4, 5, 6, 7, and  8)
have a high disagreement. Let's call this dataset HD. This determines the
maximum percentage of high disagreement tweets that could be in LD. In other
words, this determines the values on the x-axis in the AUC curves when
increasing the % of tweets with low agreement in the training set.
3. Perform experiment 3 using 4 labels per tweet, i.e.
start with LD and gradually replace some of those tweets with some from HD.
We plot the resulting average AUC curves just like in experiment 3.
4. Repeat step 3 using 5, 6, 7, and 8 labels per tweet respectively. This will
give us 4 more average AUC curves.
5. Then we can compare the difference of the average AUC curves: AUC with 0%
tweets with high disagreement vs. AUC with X% tweets with high disagreement,
where X is |HD| / |LD|, i.e.  the number of tweets with high disagreement
divided by the number of low disagreement.

This script identifies |HD| and |LD| and creates the initial datasets for LD
(containing only tweets that always have low disagreement using 4-8 labels in
LOW) and HD (containing only tweets that always have high disagreement using
4-8 labels in LOW) from which Gizem creates datasets.
"""
import os
import re
import codecs
...
def export_to_csv_low(
    ...
    if not os.path.isfile(glove_src):
        ...
        merged = merge_dicts(tweets, watson, rosette)
        dst = "/media/data/Workspaces/PythonWorkspace/phd/Analyze-Labeled-" \
              "Dataset/www2018_results/dataset_twitter_agreement_experiment/" \
              "crowd_train_merged_{}_labels.json".format(first_n)
        store_json(merged, dst)
        df = read_as_df(dst, tids)
        store_glove_vectors_for_dataset(GLOVE_VEC_TWITTER, df,
...
def keep_consistent_labels(rounds, label):
    """
    Keeps only tweets that have <label> as majoritiy label using any number
    of labels used in <rounds>.

    Parameters
    ----------
    rounds: list of dicts - each dict contains all tweets of an iteration, where
    each tweet's label was determined using majority voting.
    label: The label which tweets should consistently keep during all iterations
    ...
    """
    # Store tweets with consistent labels
    res = {}
    first = rounds[0]
    rest = rounds[1:]
    for tid in first:
        # If tweet has desired label
        if first[tid][LABEL_CLA] == label:
            # Check label of tweet in each iteration
            for iteration in range(len(rest)):
                # Get label of <iteration>-th tweet
                tweet = rest[iteration][tid]
                # Tweet has an inconsistent label, skip other iterations
                if tweet[LABEL_CLA] != label:
                    break
                # Tweet label is consistent even in last iteration
                if tweet[LABEL_CLA] == label and iteration == len(rest) - 1:
                    res[tid] = first[tid]
    return res


def merge_tweets(tweets, dst, csv_dir, tfidf_dir, rosette_dir, watson_dir,
                 rosette_dir_train, watson_dir_train, glove_src, use_tfidf,
                 features, first_n):
    """
    Merges the specified tweets with computed features and stores them in json
    format.
...
def export_to_csv_sd_ld(
        low, dst_dir, csv_dir, tfidf_dir, rosette_dir,
        watson_dir,
        rosette_dir_train, watson_dir_train, glove_src, use_tfidf, features,
        first_n, use_tweets):
    """
    Creates dataset for HIGH using only tweets that don't change their
    disagreement labels when using 4-8 labels for majority voting.

    Parameters
    ----------
    low: str - path to LOW crowdsourced dataset.
    dst: str - directory where dataset will be stored.
    csv_dir: str - path to directory in which csv files will be stored.
    tfidf_dir: str - path to directory in which the tfidf_weights will be stored
    rosette_dir: str - path where the features extracted via Rosette are stored.
    watson_dir: str - path where the features extracted via Watson are stored.
    ...
    classifier.
    first_n: int - number of first labels to use from crowdsourcing datasets.
    use_tweets: int - number of tweets with low disagreement to be used. It
    could be either 87 or 2*87 = 174 depending on the experiment setup.

    """
    # Number of tweets with low disagreement, i.e. tweets with high agreement
    LD = use_tweets
    # Path where tweets with low agreement will be stored
    fname_la = "twitter_agreement_low_low_agree_{}_{}_labels.json" \
        .format(LD, first_n)
    dst_la = os.path.join(dst_dir, fname_la)
    # Path where tweets with high agreement will be stored
    fname_ha = "twitter_agreement_low_high_agree_{}_{}_labels.json" \
        .format(LD, first_n)
    dst_ha = os.path.join(dst_dir, fname_ha)

    # Contains tweets per considered labels, i.e. 5 entries
    rounds = []
    # Now only keep tweets whose labels won't change - sufficiently
    for considered_labels in [4, 5, 6, 7, 8]:
        tweets = read_amt_csv(low, considered_labels, 8)
        rounds.append(tweets)
    # "low" means low agreement = high disagreement, HD
    la = keep_consistent_labels(rounds, "low")
    # "high" means high agreement = low disagreement, LD
    ha_ = keep_consistent_labels(rounds, "high")
    print "#HD", len(la)
    print la
    print "#LD", len(ha_)
    print ha_

    # Sanity check to make sure no tweets are added here erroneously
    for tid in ha_:
        for round in rounds:
            assert(round[tid][LABEL_CLA] == "high")
    for tid in la:
        for round in rounds:
            assert(round[tid][LABEL_CLA] == "low")

    # Sample tweets with high agreement because there are more of them
    # than those with low agreement
    selected = random.sample(ha_.keys(), LD)
    print "selected tweets with high agreement", len(selected), selected
    ha = {}
    for tid in selected:
        ha[tid] = ha_[tid]
    ...
    ##################################
    # Use this to commented code to create datasets for Gizem
    # Comment out the actual merge_tweets() function, otherwise Gizem's files
    # are overridden
    ###################################
    # # Store tweets with low agreement
    # la_tmp = copy.deepcopy(la)
    # file_name = fname_la.split(".")[0]
    # dst_gizem = os.path.join(csv_dir, "{}.csv".format(file_name))
    # with open(dst_gizem, "wb") as f:
    #     writer = csv.writer(f, delimiter="\t")
    #     for tid in la_tmp:
    #         writer.writerow([tid, la_tmp[tid][TEXT]])
    #
    # # Store tweets with high agreement
    # ha_tmp = copy.deepcopy(ha)
    # # Use this to commented code to create datasets for Gizem
    # file_name = fname_ha.split(".")[0]
    # dst_gizem = os.path.join(csv_dir, "{}.csv".format(file_name))
    # with open(dst_gizem, "wb") as f:
    #     writer = csv.writer(f, delimiter="\t")
    #     for tid in ha_tmp:
    #         writer.writerow([tid, ha_tmp[tid][TEXT]])
    #################################

    # Store LA
    merge_tweets(la, dst_la, csv_dir, tfidf_dir, rosette_dir, watson_dir,
                 rosette_dir_train, watson_dir_train, glove_src, use_tfidf,
                 features, first_n)

    # Store HA
    merge_tweets(ha, dst_ha, csv_dir, tfidf_dir, rosette_dir, watson_dir,
                 rosette_dir_train, watson_dir_train, glove_src, use_tfidf,
                 features, first_n)
...
if __name__ == "__main__":
    ...
    use_tfidf = True
    #######################################################################
    # 1. Count #tweets with low/high disagreement if we use 4-8 labels.   #
    # We only count tweets that don't change their label ever.            #
    # Use 87 tweets with low and high disagreement each                   #
    #######################################################################
    first_n = 8
    use_high_agree_tweets = 87

    export_to_csv_sd_ld(LOW, DST_DIR, CSV_DIR, TFIDF_DIR, ROSETTE_DIR,
                        WATSON_DIR, ROSETTE_DIR_TRAIN, WATSON_DIR_TRAIN,
                        GLOVE_W2V_VEC_TWITTER, use_tfidf, FEATURES, first_n,
                        use_high_agree_tweets)

    #######################################################################
    # 2. Count #tweets with low/high disagreement if we use 4-8 labels.   #
    # We only count tweets that don't change their label ever.            #
    # Use 174 tweet with low and 87 tweets with high disagreement         #
    #######################################################################
    use_high_agree_tweets = 174
