...
It's used for Gizem's experiment that investigates the influence of low
agreement tweets on the resulting classifier performance. That experiment uses
the training set as well as the 3 crowdsourced datasets.
...
# Gizem chose these labels
SENTIMENT_LEVELS = ["Neut", "Neg", "Pos", "Irrel"]
...
def read_crowdsourcing_ds():
    ...
    """
    crowd_dir = os.path.join(base_dir, "www2018_results", "crowdsourcing_datasets")
    tids = {}
...
def export_to_csv(train, ds_dir, dst_dir, tfidf_dir, json_dir, rosette_dir,
                  watson_dir, rosette_dir_train, watson_dir_train,
                  glove_src, use_tfidf, features):
    """
    ...
    if not os.path.isfile(glove_src):
        ...
        dst = os.path.join(base_dir, "www2018_results", "dataset_twitter",
...
if __name__ == "__main__":
    ...
    k = 2200
    DS = os.path.join(base_dir, "results", "dataset_twitter",
                      "dataset_twitter_watson_rosette_cleaned.json")
    # Directory in which the datasets are stored
    GIZEM_DIR = os.path.join(base_dir, "results", "dataset_gizem",
    ...
    CSV_DIR = os.path.join(base_dir, "results", "dataset_gizem",
                           "csv_files_{}".format(k))
    # Directory in which computed TF-IDF vectors will be stored for the
    # different datasets
    TFIDF_DIR = os.path.join(base_dir, "results", "dataset_gizem",
                             "tf_idf_{}".format(k))
    ...
    # Rosette/Watson directory for training set
    ...
    JSON_DIR = os.path.join(base_dir, "results", "dataset_gizem",
                            "json_{}".format(k))
    ...
    # And create dataset once using Twitter glove vectors and merging
    # TRAIN with LOW, MEDIUM, and HIGH
    # 1. Perform experiment using HA = 2.2k tweets, i.e. dataset size is 2.2k
    # tweets and LA = 1.1k so maximum % of tweets with low agreement will be
    # 50% in the dataset
    export_to_csv(DS, GIZEM_DIR, CSV_DIR, TFIDF_DIR, JSON_DIR, ROSETTE_DIR,
                  WATSON_DIR, ROSETTE_DIR_TRAIN, WATSON_DIR_TRAIN,
                  GLOVE_W2V_VEC_TWITTER, use_tfidf, FEATURES)

    # 2. Perform experiment using HA = 1.1k tweets, i.e. dataset size is 1.1k
    # tweets and LA = 1.1k so maximum % of tweets with low agreement will be
    # 100% in the dataset
    k = 1100
    GIZEM_DIR = os.path.join(base_dir, "results", "dataset_gizem",
    ...
    # 1. Perform experiment using HA = 2.2k tweets, i.e. dataset size is 2.2k
    # tweets so maximum % of tweets with low agreement will be 50% in the
    # dataset
    ...
    CSV_DIR = os.path.join(base_dir, "results", "dataset_gizem",
                           "csv_files_{}".format(k))
    FEATURE_DIR = os.path.join(base_dir, "results", "dataset_gizem",
                               "FeatureFiles_{}".format(k))
    # Path to header file containing the feature names and their order
    HEADER = os.path.join(base_dir, "results", "dataset_gizem",
                          "Features_Trump.txt")
    ARFF_DIR = os.path.join(base_dir, "results", "dataset_gizem",
                            "arff_files_{}".format(k))
    if not os.path.exists(ARFF_DIR):
        os.makedirs(ARFF_DIR)
    export_to_arff(CSV_DIR, FEATURE_DIR, HEADER, ARFF_DIR, FEATURES)

    # 2. Perform experiment using HA = 1.1k tweets, i.e. dataset size is 1.1k
    # tweets so maximum % of tweets with low agreement will be 100% in the
    # dataset
    #  Path to Gizem's extracted features
    k = 1100
    # Directory in which the converted .csv files are stored
