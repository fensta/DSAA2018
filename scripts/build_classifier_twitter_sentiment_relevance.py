...
This script addresses part 1 of the sentiment prediction task.
Weka prediction results are stored under
weka_predictions/sentiment_relevance_predictions_twitter
...
# Labels in arff file for relevance prediction part of sentiment analysis
RELEVANCE_LEVELS = ["Relevant", "Irrelevant"]
...
if __name__ == "__main__":
    ...
    # # Create dataset once using Wikipedia glove vectors
    # fname = "dataset_twitter_wikipedia_classification.arff"
    # dst = os.path.join(ARFF_DIR, fname)
    # export_to_weka(src, GLOVE_W2V_VEC_WIKI, use_tfidf, dst, FEATURES, LABEL_CLA)

    # And create dataset once using Twitter glove vectors
    fname = "dataset_twitter_relevance_sentiment_twitter_classification.arff"
    dst = os.path.join(ARFF_DIR, fname)
    export_to_weka(src, GLOVE_W2V_VEC_TWITTER, use_tfidf, dst, FEATURES,
                   LABEL)
    ...
    fname = "dataset_twitter_full_watson_rosette_cleaned.json"
    ...
    if not os.path.isfile(GLOVE_W2V_VEC_TWITTER):
        store_glove_vectors_for_dataset(GLOVE_VEC_TWITTER, src,
                                        GLOVE_W2V_VEC_TWITTER)
    if not os.path.isfile(GLOVE_W2V_VEC_WIKI):
        store_glove_vectors_for_dataset(GLOVE_VEC_WIKI, src,
                                        GLOVE_W2V_VEC_WIKI)
    ...
    #############################################################
    # And create dataset once using Twitter glove vectors
    fname = "dataset_twitter_full_relevance_sentiment_twitter_classification" \
            ".arff"
    dst = os.path.join(ARFF_DIR, fname)
    export_to_weka(src, GLOVE_W2V_VEC_TWITTER, use_tfidf, dst, FEATURES,
                   LABEL)
